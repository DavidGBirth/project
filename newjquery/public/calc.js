...
$(".numbers button").on('click', function () {
	...
	if (digit === "equal" && !first) {
		let value2 = parseInt(visor.text());
		calc.setOperand2(value2);
		visor.html(calc.getResult());
...
$(".symbols button").on('click', function () {
	if (visor.length && first) {
		let value1 = parseInt(visor.text());
		calc.setOperand1(value1);
		calc.setOperation($(this).val());
		visor.html("");
		first = false;
	}
});
